...
def parseModel(model_xml, urdf_filepath):
    print("--------------------")
    print("parsing model")
    ...
    if include_xml is not None:
      ...
      if uri_xml is not None:
        ...
        if uri.startswith(prefix):
          uri = urdf_filepath +"/models/" + uri[len(prefix):]+"/model.sdf"
        print("uri=", uri)
        newmodel['uri'] = uri
...
def parseWorld(p, filepath, model_path):
  ...
  for model_xml in root.iter('model'):
      ...
      newmodel = parseModel(model_xml, model_path)
